...
export class WordTypingComponent implements OnInit, OnDestroy {
  contentExample = [];
  information = [];
  wordTypingOption: ContentSetting = {timeLimit: 5, isTest: false,
  ...
  oneStepClicked(test?) {
    ...
    this.contentService.startStep(1, baseData.baseWord);
  }
  twoStepClicked(test?) {
    this.information = ['이번엔 조금 더 어려운 단어를 연습해보자', '조금 어려우면 시간을 늦춰도 좋아.'];
    this.contentService.startStep(2, baseData.secondWord);
  }
